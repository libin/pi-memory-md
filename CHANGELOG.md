...
## [Unreleased] (libin fork)

### Added
- Optional `status` frontmatter on memory files (`verified | done | in-progress | failed | superseded`), surfaced next to each file in the delivered memory index. The context header now tells the agent to treat status as the outcome of past work and to never silently repeat work marked `done`/`verified` or retry an approach marked `failed`. `memory-write.sh` gains an optional `status` argument on `create` and a `set-status` subcommand. Idea borrowed from claude-code-project-brain's status-with-outcome convention.
